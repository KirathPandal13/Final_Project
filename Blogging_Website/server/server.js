...
const deleteComments = ( _id ) => {
    ...
    .then(comment => {
        ...
        if(comment.parent){
            Comment.findOneAndUpdate({ _id: comment.parent }, { $pull: { children: _id } }).then(data => console.log('comment delete from parent'))
...
server.post('/update-profile', verifyJWT, (req, res) => {
    ...
    if(username.length < 3) {
        return res.status(403).json({ error: "You must fill username with more than 3 letters"});
    } 
    if(bio.length > 200){
...
server.post("/change-password", verifyJWT, (req, res) => {
    ...
    .then((user) => {
        ...
        if(user.google_auth){
            return res.status(500).json({ error: "You can't change account's password because you logged in through google" })
        }
        
        bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
            if (err) {
                return res.status(500).json({ error: "Some Error occure while changing password, please try again" })
            }
...
server.post("/get-blog", (req, res) => {
    ...
    .populate("author", "personal_info.username personal_info.profile_img personal_info.fullname")
...
server.post('/add-comment', verifyJWT, (req, res) => {
    ...
    let commentObj = {
        blog_id: _id, blog_author, comment, commented_by: user_id, isReply: Boolean(replying_to)
    }
